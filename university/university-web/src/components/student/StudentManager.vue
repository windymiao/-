<template>
    <div>
      ...
      <div style="margin-bottom: 5px">
        ...
        <el-select v-model="deptId" placeholder="请选择系别" style="margin-left: 5px;" @change="resetMajor">
          <el-option
        ...
        <el-select v-model="majorId" placeholder="请选择专业" style="margin-left: 5px;">
          <el-option
            v-for="item in filteredMajorData"
            :key="item.majorId"
    ...
    <el-dialog
      ...
      <el-form ref="stuForm" :model="stuForm" :rules="stuRules" label-width="80px">
        ...
        <el-form-item label="系别" prop="deptId">
            <el-col :span="20">
              <el-select v-model="stuForm.deptId" placeholder="请选择系别" style="margin-left: 5px;" @change="resetStuFormMajor">
                <el-option
                  v-for="item in deptData"
                  :key="item.deptId"
                  :label="item.deptName"
        ...
        <el-form-item label="专业" prop="majorId">
            <el-col :span="20">
              <el-select v-model="stuForm.majorId" placeholder="请选择专业" style="margin-left: 5px;">
                <el-option
                  v-for="item in filteredStuFormMajorData"
                  :key="item.majorId"
                  :label="item.majorName"
                  :value="item.majorId">
                </el-option>
      ...
      computed: {
        filteredMajorData() {
          return this.majorData.filter(item => {
            if(this.majorData != null || this.deptId!='' || this.deptId!=null){
              // 此处要使用 == 进行判断，因为 item.deptId 不是一个序列类型，是一个int整型
              // return item.deptId.includes(this.deptId);
              return item.deptId==this.deptId;
            }
            return this.majorData;
          });
        },
        filteredStuFormMajorData() {
          return this.majorData.filter(item => {
              return item.deptId==this.stuForm.deptId;
          });
        },
      },
      methods:{
        ...
        // 当查询条件中的系别发生改变时，将对应专业清空
        resetMajor(){
          this.majorId=""
        },
        // 当新增表单中的系别发生改变时，将对应专业清空
        resetStuFormMajor(){
        ...
        //  将major名字，回显到页面  (根据deptId,dept若为空,则显示所有.)
        formatMajor(row){
            let res = this.majorData.find(item=>{

              // if(this.deptId!=null || this.deptId != ''){
              //   return item.majorId == row.majorId && item.deptId == this.deptId
              // }
        ...
        // 出现的问题: 使用了学号是否存在作为区分，但添加学生信息的时候，需要在模态框中输入学号，此时学号一定不为空，所以一直进入
        // 修改逻辑，导致添加学生信息失败
        saveStu(){
          // 判断是否通过了rules规则
