        ...
        <el-table-column prop="operate" label="操作">
          <template slot-scope="scope">
            <el-popconfirm
              ...
              <el-button slot="reference" size="small" type="success" style="margin-left: 5px;">选择</el-button>
            </el-popconfirm>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  export default {
      ...
      methods:{
        ...
        selectCourse(row){
            console.log(row)
            this.$axios.post(this.$httpUrl+"/sc/save",{
        ...
        // 加载所有的课程成绩信息
        loadPost(){
          this.$axios.post(this.$httpUrl+'/course/getCourse',{
            pageSize : this.pageSize,
            pageNum : this.pageNum,
            param:{
                sno:this.student.sno,
                deptId:this.student.deptId
            }
          }).then(res=>res.data).then(res=>{
            console.log(res)
            if(res.code == 200){
